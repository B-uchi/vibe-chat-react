...
const Settings = ({ currentUser, updateCurrentUser }) => {
  ...
  const updateProfile = async () => {
    ...
    if (profilePhoto) {
      const storageRef = ref(storage, `kyc/${currentUser.id}.${profilePhoto.type.split("/")[1]}`)
    ...
    const response = await fetch("http://localhost:5000/api/user/updateProfile", {
      method: "PATCH",
      body: JSON.stringify({ username, bio, profilePhotoUrl: uploadedPhotoUrl }),
      headers: {
  ...
  // implement a skeleton loader for chats
    ...
    <div className='bg-[#efefef] h-full flex justify-center items-center lg:p-0 p-2'>
      ...
      <div className="border-[1px] border-[#e1e1e1] lg:w-1/2 rounded-md p-3 bg-white h-[80%] flex">
        <div className="p-3 border-r-[1px] border-[#e1e1e1] lg:w-[30%] h-full flex flex-col items-start gap-2">
          ...
          <button className={`flex p-3 items-center gap-3 hover:bg-[#efefef] w-full rounded-md ${activeButton == "privacy" ? " bg-[#efefef] " : ""}`}><RiGitRepositoryPrivateFill size={22} /> <span className='lg:block hidden'>Privacy</span> </button>
        ...
        <div className="h-full flex-grow p-3 overflow-scroll">
          {page === 'profile' && (
            <div className="flex flex-col gap-3">
              ...
              <div className="">
                {currentUser && currentUser.profileData.profilePhoto ? <div style={{ backgroundImage: `url(${profilePhotoUrl ? profilePhotoUrl : currentUser.profileData.profilePhoto})` }} className="mx-auto bg-cover bg-no-repeat bg-center rounded-full w-[120px] h-[120px] border-[1px] border-[#e1e1e1] bg-[#efefef] flex items-center justify-center cursor-pointer"><button onClick={() => selectImage()} className='bg-white bg-opacity-50 rounded-full p-2'><CiCamera size={30} /> <input
                  type="file"
                  accept="image/*"
                  ref={fileInputRef}
                  onChange={validateImage}
                  className="hidden"
                /></button></div> : <div onClick={() => selectImage()} className="mx-auto bg-[#efefef] w-[120px] h-[120px] rounded-full flex items-center justify-center cursor-pointer"><CiCamera size={30} /></div>}
              </div>
              <div>
              ...
              <button onClick={() => updateProfile()} className='bg-[#333] text-white p-2 rounded-md flex items-center justify-center gap-3'>Save {loading && <div className="loader"></div>}</button>
            </div>
          )}
          {page === 'security' && (
            <div className="flex flex-col gap-3">
